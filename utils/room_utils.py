...
import matplotlib.pyplot as plt
from ..utils.direction_utils import hierarchy_resample, bootstrap_sample, get_sample_info
from ..utils.file_utils import load_json, load_jsonl
from ..utils.file_utils import my_colors

connectivity_work_dir = pathlib.Path('/Users/zijiao/home/research/Matterport3DSimulator/connectivity')
...
def process_result_n_hop(results, datasets, scanregion=None, argmax=False, **kwargs):
    ...
    if not scanregion:
        with open('/Users/zijiao/home/research/unit-vln/code/region_data/scan_region.json') as t:
    ...
    for item in result_intervene:
        ...
        _, dist_stop_to_goal = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, stop_viewpoint)
        ...
        _, dist_intervene_to_goal_final = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, final_viewpoint)
        ...
        _, dist_no_intervene_to_goal_final = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, no_intervention_item['final_viewpoint_id'][0])
        ...
        def get_cand_dist_and_prob(item, goal_viewpoints_lst):
            ...
            for i, cand in enumerate(item['candidates'][0][1]):
                ...
                _, cand_dist_intervene_to_goal_next = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, cand_viewpoint)
                dist_to_goal_next_and_prob_pairs.append((cand_dist_intervene_to_goal_next, cand_prob))
            return dist_to_goal_next_and_prob_pairs

        if argmax:
            _, dist_intervene_to_goal_next = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, next_viewpoint)
            dist_to_goal_intervenes.append(dist_intervene_to_goal_next)
            _, dist_no_intervene_to_goal_next = closest_distance(scan_id, shortest_distances, goal_viewpoints_lst, no_intervention_item['next_pos'])
            ...
            next_delta_no_intervene = - (dist_no_intervene_to_goal_next - dist_stop_to_goal)
            ...
            final_delta_no_intervene = -(dist_no_intervene_to_goal_final - dist_stop_to_goal)
            final_delta_intervenes.append(final_delta_intervene)
            final_delta_no_intervenes.append(final_delta_no_intervene)

            roomtype2delta[room_type].append((next_delta_intervene, next_delta_no_intervene, final_delta_intervene, final_delta_no_intervene))
            
        else:
            d2g_intervene = get_cand_dist_and_prob(item, goal_viewpoints_lst)
            dist_to_goal_intervenes.extend(d2g_intervene)
            d2g_no_intervene = get_cand_dist_and_prob(no_intervention_item, goal_viewpoints_lst)
...
def draw_distribution_on_delta_to_goal_room(result_data, title='data distribution by distance to goal', hop='', filetype='png', save_dir='', argmax=False, maxValue=None,
                                      delta=False, bootstrap=False):
    # *
    ...
    if delta:
        ...
        x_deltas = np.arange(-5, 5, 0.5).tolist() #? this delta distance x axis is too specific is there a better way?
        rr = (-5,5)
        maxys = []
        barWidth = 0.3
        ...
        for i in range(100):
            ...
            bin_number, bins = np.histogram(intervene_next_delta, bins=x_deltas, range=rr, density=True)
            intervene_bin_numbers_collection.append(bin_number.tolist())

            no_intervene_next_delta = boot_sample['no_intervene_delta']
            no_intervene_mean_collection.append(np.mean(boot_sample['no_intervene_delta']))
            bin_number, bins = np.histogram(no_intervene_next_delta, bins=x_deltas, range=rr, density=True)
        ...
        for i, (bin_numbers_collection, mean_collection, delta_name, color, zcolor, bar_pos_set) in enumerate(zip([intervene_bin_numbers_collection, no_intervene_bin_numbers_collection],
                                                                                                zip([intervene_mean_collection, no_intervene_mean_collection]),
                                                                                                delta_names,
                                                                                                bar_colors, zero_bucket_colors, bar_pos_sets)):
            ...
            ax.bar(x+bar_pos_set, datas, yerr=yerror, width=barWidth, color=color, label='Nonzero Delta' , capsize=7, error_kw=dict(ecolor='black', ls='-', lw=2, capsize=5, capthick=2))
        ...
        legend_patches = [patches.Patch(color=colors[j], label=labels[j], alpha=0.5) for j in range(2)]
        ax.legend(handles=legend_patches, loc='upper center', bbox_to_anchor=(0.5, 1.15), ncol=2)  
...
def create_exact_x_hop_dataset_with_no_region_loop(no_end, result_no_end,x, scan_viewpoint_label, with_stop=False):
    # *
    ...
    for inference in result_no_end:
        ...
        cand_ids = [inf_cand['viewpointId'] for inf_cand in inference['candidates'][0][1]]
        stop_viewpoint_id = inference['candidates'][0][0]

        # ! not used in current version
        cand_ids = cand_ids + [stop_viewpoint_id]
        for i, (target, path) in enumerate(weight_1_shortest_paths[scan_id][stop_viewpoint_id].items()):
            ...
            if len(path) <= x + 1 and target_region_label != stop_region_label and target_region_label:#  and target not in cand_ids:

                region2hop_nums[target_region_label].append(len(path)-1)
                inter_id2nhop_region_label[instr_id][target_region_label].append((i, target))
    ...
    for instr_id, regions in inter_id2nhop_region_label.items():
        ...
        for region_label, target in regions.items():
            if region_label not in 'zZ':
                ...
                intervene_item['instruction_id'] = intervene_item['instruction_id'] + '_' + region_name
                intervene_item['instruction'] = append_room_instruction(intervene_item['instruction'], region_name, with_stop=with_stop)
...
def load_hop_data_and_results(hops, data_dir, result_dir, args):
    ...
    for hop in hops:
        hop_data = load_jsonl(f'{data_dir}/{hop}_hop/rxr_{hop}_hop_intervene_dataset.jsonl'), load_jsonl(f'{data_dir}/{hop}_hop/rxr_{hop}_hop_no_intervene_dataset.jsonl')
        hop_data_list.append(hop_data)

        if 'envdrop' in args.model_name:
            hop_result = load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_intervene_dataset.json'), load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_no_intervene_dataset.json')
            hop_result_list.append(hop_result)
        else:
            hop_result = load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_intervene_dataset_0.json'), load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_no_intervene_dataset_0.json')
            hop_result_list.append(hop_result)
...
def generate_hop_results(hops, hop_data_list, hop_result_list, delta=False, **kwargs):
    ...
    for idx, hop in enumerate(hops):
        ...
        hop_processed = process_result_n_hop(hop_result, hop_data, argmax=False, **kwargs)
        ...
        dist_to_goal_no_intervenes_next = hop_processed['dist_to_goal_no_intervene_next'] 
        dist_to_goal_intervenes_final = hop_processed['dist_to_goal_intervene_final']
        dist_to_goal_no_intervenes_final = hop_processed['dist_to_goal_no_intervene_final'] 
        ...
        next_delta_intervene, next_delta_no_intervene = hop_processed['next_delta_intervene'], hop_processed['next_delta_no_intervene']
        final_delta_intervene, final_delta_no_intervene = hop_processed['final_delta_intervene'], hop_processed['final_delta_no_intervene']
        ...
        if hop == 1:
            ...
            dist_to_goal_intervenes_final = [x['dist2goal'] for x in dist_to_goal_intervenes_final]
            dist_to_goal_intervene_collects.extend(dist_to_goal_intervenes_final)
            # dist_to_goal_no_intervenes_final['hop'] = hop

            dist_to_goal_no_intervenes_final = [x['dist2goal'] for x in dist_to_goal_no_intervenes_final]
        ...
        if delta:
            hop_results = pd.DataFrame({'hop': hop_nums, 'delta_in': delta_intervene_collects, 'dist_to_goal_no_intervene': delta_no_intervene_collects,}) #'dist_to_goal_stops': dist_to_goal_stops_collects})
        else:
            ...
            hop_results = {'hop': hop_nums, 'dist_to_goal_intervene': dist_to_goal_intervene_collects, 'dist_to_goal_no_intervene': dist_to_goal_no_intervene_collects, 'dist_to_goal_stops': dist_to_goal_stops_collects}
    return hop_results
...
def create_ridge_plot(hops, hop_results, data_dir, filename, delta=False, **kwargs):
    ...
    for i, name in enumerate(names):
        g.map(custom_kdeplot, 'x' ,name , color=colors[i], label=labels[i], alpha=0.3, linewidth=3.0)

    xmin, xmax = rg #(-5, 30)
    g.map(plt.axhline, y=0,
        lw=2, clip_on=False, xmin=0, xmax=1, color='k')

    for i, ax in enumerate(g.axes.flat): # type: ignore
        ax.text(-5, 0.02, hops[i],
    ...
    if len(hops) == 1:
        if delta:
              g.set_xlabels("Delta Distances to Goal (Relative to Start Position)", labelpad=15)
        else:
    ...
    g.set_ylabels("")
    if not delta:
        legend_patches = [patches.Patch(color=colors[j], label=labels[j], alpha=0.5) for j in range(3)]
        # legend_patches = [mlines.Line2D([], [], color=colors[j], label=labels[j], alpha=0.9, linewidth=3.5) for j in range(3)]
    else:
        legend_patches = [patches.Patch(color=colors[j], label=labels[j], alpha=0.9) for j in range(2)]

    if not delta:
        g.add_legend(handles=legend_patches, ncol=3, bbox_to_anchor=(0.31, 1.05), loc='center')
    else:
        g.add_legend(handles=legend_patches, ncol=2, bbox_to_anchor=(0.3, 1.05), loc='center')
    # plt.show()
