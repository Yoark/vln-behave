...
def build_dataset(args, rank=0, is_test=False):
    ...
    if args.submit:
        if args.dataset == 'r2r':
            ...
            if args.ablate_names:
                val_env_names = args.ablate_names
            else:
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    rxr_data = ['val_seen', 'val_train_seen', 'test_standard_public', 'test_challenge_public']
    for env_name, env in val_envs.items():
        if os.path.exists(os.path.join(args.pred_dir, "submit_%s.json" % env_name)) and args.pred_dir in rxr_data:
            continue
        ...
        if default_gpu:
            ...
            if args.ablate_names:
                for item in preds:
                    ...
                    scan, gt_traj = env.gt_trajs[instr_id]
                    traj_scores = env._eval_item(scan, traj, gt_traj)
                    item['success'] = traj_scores['success']

            if args.submit:
                json.dump(
                    preds,
                    open(os.path.join(args.pred_dir, f"submit_{env_name}_{str(int(args.store_prev_stops))}.json"), 'w'),
