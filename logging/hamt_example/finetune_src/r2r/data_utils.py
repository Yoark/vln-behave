    ...
    for split in splits:
        if "/" not in split:    # the official splits
            if dataset == 'r2r':
                ...
                new_data = []
                filename = ''
                if args.ablate_names and split != 'train':
                    filename = 'rxr_%s.jsonl'%split
                    if args.experiment_folder:
                        filename = os.path.join(args.experiment_folder, filename)
                else:
                    filename = 'rxr_%s_guide_enc_xlmr.jsonl'%split 
                with open(os.path.join(anno_dir, filename), 'rb') as f:
                    for item in f:
...
def construct_instrs(anno_dir, dataset, splits, tokenizer=None, max_instr_len=512):
    ...
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits)):
        if dataset == 'rxr':
            ...
            new_item = dict(item)
            if 'path_id' in item:
                new_item['instr_id'] = '%s_%s'%(str(item['path_id']), str(item['instruction_id']))
            else: # test
                new_item['path_id'] = new_item['instr_id'] = str(item['instruction_id'])
            # this might create a bug if </s> is truncated and it is the longest sample in the batch...
            if 'instr_encoding' in item:
                new_item['instr_encoding'] = item['instr_encoding'][:max_instr_len]
            else:
                
                tokenized = tokenizer.tokenize(new_item['instruction'])
                trunc = tokenized[:max_instr_len-2]
                sequence = ['<s>'] + trunc + ['</s>']
                tokenized_id = tokenizer.convert_tokens_to_ids(sequence)
                new_item['instr_encoding'] = tokenized_id
            ...
            for j, instr in enumerate(item['instructions']):
                new_item = dict(item)
                new_item['instr_id'] = '%s_%s' % (str(item['path_id']), j)
