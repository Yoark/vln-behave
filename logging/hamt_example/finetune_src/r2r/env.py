...
import sys
sys.path.append('/usr/src/env_drop/build/')
import MatterSim
...
class R2RBatch(object):
    ...
    def make_candidate(self, feature, scanId, viewpointId, viewId):
        ...
        base_heading = (viewId % 12) * math.radians(30) # agent's current heading
        adj_dict = {}
        ...
        if long_id not in self.buffered_state_dict:
            for ix in range(36):
                ...
                heading = state.heading - base_heading # the difference between new episode agent heading and current heading
                ...
                for j, loc in enumerate(state.navigableLocations[1:]):
                    ...
                    loc_heading = heading + loc.rel_heading # the difference between candidate and current heading
                    ...
                    if (loc.viewpointId not in adj_dict or
                        ...
                        adj_dict[loc.viewpointId] = {
                            ...
                            # 'position': self.graphs[scanId].nodes[loc.viewpointId]['position'].tolist(),
                        }
            ...
            for c in candidate:
                c_new = c.copy()
                ix = c_new['pointId']
                normalized_heading = c_new['normalized_heading'] # normalized heading is to ease reuse of the same scan+viewpoint
                visual_feat = feature[ix]
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            obs.append({
                ...
                'teacher' : self._teacher_path_action(state, item['path'], t=t, shortest_teacher=shortest_teacher), #* this will equal to current viewpoint if longer than item['path']
                'gt_path' : item['path'],
                'path_id' : item['path_id'],
                # 'position': self.graphs[state.scanId].nodes[state.location.viewpointId]['position'].tolist(),
                # 'goal_region_loc': item['region_loc']
            })
