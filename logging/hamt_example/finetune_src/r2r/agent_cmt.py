...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    # Use non-batched version of simulator
    ...
    def _cand_pano_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            for j, cc in enumerate(ob['candidate']):
                ...
                cand_pointids[cc['pointId']] = True #pointId is 1 in 36
    ...
    def _teacher_action_inference(self, obs, ended):
        """
        Extract teacher actions into variable.
        :param obs: The observation.
        :param ended: Whether the action seq is ended
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        stopped = [0 for _ in range(batch_size)]
        for t in range(self.args.max_action_len):
            ...
            elif self.args.test_type == 'type2':
                # Do teacher forcing for inference
                ...
                for i, (teachering, a) in enumerate(zip(is_teachering, target)):
                    if teachering == 'yes':
                        ...
                        if self.args.store_prev_stops:
                            prev_stop_prob = torch.softmax(logit[i].detach(), dim=0)[ob_cand_lens[i]-1].item()
                            traj[i]['prev_stop_probs'].append((t, prev_stop_prob))
                    elif teachering == 'no':
                        a_t[i] = a_t_argmax[i]
                        # this should be the first prediction after supervision
                        if not stopped[i]:
                            stop_prob = torch.softmax(logit[i].detach(), dim=0)[ob_cand_lens[i]-1].item()
                            stopped[i] = 1
                        ...
                        if self.args.store_final_logits: # could be more than one logged
                            ...
                            traj[i]['final_logits'].append(final_logits) # now how to mapping from logits to candidates
                            log_cands = []
                            ...
                            traj[i]['nav_type'].append(ob_nav_types[i].tolist()) # why 38? since the pano feature and cand feature are appended, don't matter for me
                            if not traj[i]['heading']:
                                traj[i]['heading'] = obs[i]['heading'] # this is absolute #! bug here could affect object and direction experiment (fixed, check again! I think there is no effect whatsoever)
                                # traj[i]['normalized_agent_heading'] = 
                            if not traj[i]['viewIndex']:
                                traj[i]['viewIndex'] = obs[i]['viewIndex']
                            # import ipdb; ipdb.set_trace()
                            # if not traj[i]['current_pos']: # check existence
                                # traj[i]['current_pos'] = self.env.graphs[obs[i]['scan']].nodes[obs[i]['viewpoint']]['position'].tolist()

                            if not traj[i]['next_pos']:
                                if a_t[i] == -1 or a_t[i] == (ob_cand_lens[i]-1) or ended[i] or a_t[i] == self.args.ignoreid:
                                    next_viewpoint = obs[i]['viewpoint']
                                else:
                                    next_viewpoint = obs[i]['candidate'][a_t[i]]['viewpointId'] 
                                traj[i]['next_pos'] = next_viewpoint
                                # traj[i]['next_pos'] = self.env.graphs[obs[i]['scan']].nodes[next_viewpoint]['position'].tolist()
                        # import ipdb;ipdb.set_trace()
            ...
            for i, next_id in enumerate(cpu_a_t):
                if next_id == (ob_cand_lens[i]-1) or next_id == self.args.ignoreid or ended[i]:    # The last action is <end>
                    ...
                    # traj[i]['final_pos'] = self.env.graphs[obs[i]['scan']].nodes['viewpoint']
        ...
        for i, item in enumerate(traj):
            # import ipdb; ipdb.set_trace()
            # item['final_pos'] = self.env.graphs[obs[i]['scan']].nodes[item['path'][-1][0]]['position'].tolist()
            item['final_viewpoint_id'] = item['path'][-1]
            # item['goal_region_loc'] = obs[i]['goal_region_loc']
            # [i for i, it in enumerate(item['path']) if obs[i]['gt_path'][-1]
        return traj
