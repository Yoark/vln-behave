...
from ..utils.direction_utils import get_sample_info, bootstrap_sample, rad2deg_mod
from ..utils.direction_utils import filter_direction_instances, get_heading_and_probs_pairs_all_directions
...
def check_success_direction(item, target):
    ...
    if argmax_index == len(item['candidates'][0][1]):
        stop = 1
        return 0, stop
    argmax_direction = rad2deg_mod(
        item['candidates'][0][1][argmax_index]['heading'])
    # print(argmax_direction)
    # ipdb.set_trace()
    success = False
    assert target in ["left", "right", "forward",
                      "backward", "back left", "back right"]
    if target == "left":
        ...
        success = (-180 < argmax_direction <= -
                   165 or 165 < argmax_direction <= 180)
...
def get_hard_success_rate(data, target, check_func=check_success_direction):
    ...
    print(f'target: {target:<10} data num: {len(data):<4} stop number: {count_stop:<4} success_num: {total_success:<4}')
...
def get_soft_success_rate(data, target):
    ...
    """
    _, direction_probs, stop_probs = get_heading_and_probs_pairs_all_directions(
        data)
...
if __name__ == '__main__':
    with launch_ipdb_on_exception():
        ...
        args.add_argument('--results_path', type=str,
                          default=None, help="the intervention results")
        args.add_argument('--baseline_path', type=str, default=None,
                          help="the baseline file path, use this if not results_path is provided")
        args.add_argument('--envdrop', action='store_true',
                          default=False, help="this is just used to change template")

        args = args.parse_args()

        directions = ['turn left', 'turn right', 'walk forward',
                      'walk backward', 'turn back left', 'turn back right']
        ...
        result_left = load_json(os.path.join(
            args.results_path, result_format.format(dir_str)))
        ...
        for dir in directions:
            ...
            for sample in bootstrap_sample_result:
                ...
                result_filtered, _ = filter_direction_instances(
                    sample, target_dir)
                mean_accum_prob = get_soft_success_rate(
                    result_filtered, target_dir)
