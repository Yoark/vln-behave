...
def check_success_object_direction(result_item, input_item):
    ...
    if argmax_index == len(result_item['candidates'][0][1]):
        return 0
    argmax_direction = result_item['candidates'][0][1][argmax_index]['normalized_heading']

    success = False
    target_direction = input_item['obj_heading'] # this is a normalized heading
    if np.abs(diff_with_direction(target_direction, argmax_direction, radians=True)) < HFOV/2:
        success = True
    return int(success)
...
def get_target_neighbor_probs(result_item, input_item):
    ...
    for i, cand in enumerate(result_item['candidates'][0][1]):
        if np.abs(diff_with_direction(target_direction, cand['normalized_heading'])) < HFOV/2:
...
if __name__ == '__main__':
    ...
    args.add_argument('--results_path', type=str, default=None, help="the intervention results")
    args.add_argument('--model_name', type=str, default='hamt', help="the model name")
    args.add_argument('--envdrop', action='store_true', default=False, help="check cli envdrop")
    ...
    submit_obj_reverie_sample = load_json(os.path.join(args.results_path, result_name_format.format('obj')))
    names = ['no_end', 'obj']
    ...
    for name in names:
        result = load_json(os.path.join(args.results_path, result_name_format.format(name)))
        ...
        for sample in bootstrap_sample_result:
            ...
            idxes = filter_instances_by_objects_heading(result, instr_id2data, fov_object=30)
            result_filtered = [instr_id2result[i] for i in idxes]
            soft_score = get_success_rate(result_filtered, instr_id2data, check_func=get_target_neighbor_probs)
