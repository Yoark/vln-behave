...
import argparse
import os
from ..utils.direction_utils import boot_sample_stop_mean, get_mean_CI_for_each_step, get_sample_info, bootstrap_sample
...
if __name__ == "__main__":
    ...
    args.add_argument('--log_path', type=str, default=None,
                      help="the score file path")
    args.add_argument('--baseline_path', type=str,
                      default=None, help="the baseline file path")
    args.add_argument('--bootstrap_num', type=int, default=100)
    args.add_argument('--envdrop', action='store_true',
                      default=False, help="check cli envdrop")
    ...
    names = ['with_end_paired_with_ahead', 'no_end_paired_with_ahead',
             'full_partial_paired_with_ahead', 'ahead_partial']
    datasets = []
    for name in names:
        datasets.append(load_json(os.path.join(
            args.results_path, result_formt.format(name))))
    ...
    instr_id2ahead_partial = {item['instr_id']
        : item for item in ahead_partial_result}
    # -------------------------start
    ...
    for sample in tqdm(bootstrap_sample_result):
        ...
        ahead_partial_collect = boot_sample_stop_mean(
            sample, instr_id2ahead_partial)
    ...
    print(f"with_end_soft: {with_end_boot['mean_over_all_steps'].iloc[0]}, no_end_soft: {no_end_boot['mean_over_all_steps'].iloc[0]}, ahead_partial_soft: {ahead_partial_boot['mean_over_all_steps'].iloc[0]}")
