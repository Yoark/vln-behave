    ...
    with open(filename) as f:
        ...
        for row in f:
            ...
            if row[0] == 'R': 
                row = row.strip().split()
                # print(row[0], row[5])
                region = {'room_type': row[5],
                        'loc_xyz': list(map(float, [row[6], row[7], row[8]])),
                        'level_index': int(row[2]),
                        'bbox': list(map(float, [row[9], row[10], row[11], row[12], row[13], row[14]])),
                        'height': float(row[15])}
...
if __name__ == '__main__':
    ...
    args.add_argument('--region_dir', type=str, default=None, help='the region data dir')
    ...
    save_json(f'{args.save_dir}/accurate_scan_viewpoint_label.json', scan_viewpoint_label)