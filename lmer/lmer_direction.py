...
from ..utils.direction_utils import rad2deg_mod, filter_direction_instances
import pandas as pd
import argparse
import os
...
def inside_checker(direction, deg):
    ...
    if direction == 'forward' and (-15 < deg <= 15):
        return 1
    if direction == 'backward' and (-180 < deg <= -165 or 165 < deg <= 180): # check this
        return 1
...
def get_data(direction, data_name, args):
    ...
    if args.model_name == 'hamt':
        result_no_end = load_json(f'{args.baseline_path}/submit_no_end_paired_with_ahead_0.json')
    else:
        result_no_end = load_json(f'{args.baseline_path}/submit_no_end_paired_with_ahead.json')
...
def get_formatted_total(direction, data_name, args):
    ...
    intervene_format = create_scan_id2intervene_label2turn_accum_prob(result_filtered, 1, direction)
    no_intervention_format = create_scan_id2intervene_label2turn_accum_prob(no_end_filtered, 0, direction)
    total = intervene_format + no_intervention_format
    return total
    
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--result_path", type=str, default="direction/data", help="Path to model result")
    parser.add_argument("--baseline_path", type=str, default="stop/data", help="Path to intervention samples")
