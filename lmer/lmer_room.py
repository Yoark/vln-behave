...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", type=str, default='delta', help="use dist to goal or delta")
    ...
    hop_data = load_jsonl(f'{data_dir}/{hop}_hop/rxr_{hop}_hop_intervene_dataset.jsonl'), load_jsonl(
            f'{data_dir}/{hop}_hop/rxr_{hop}_hop_no_intervene_dataset.jsonl')
    if 'envdrop' in result_dir:
        hop_result = load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_intervene_dataset.json'), load_json(
            f'{result_dir}/{hop}_hop/submit_{hop}_hop_no_intervene_dataset.json')
    else:
        hop_result = load_json(f'{result_dir}/{hop}_hop/submit_{hop}_hop_intervene_dataset_0.json'), load_json(
