...
def create_scan_id2intervene_label2object(input_data, data, intervene_label):
    ...
    for item in data:
        candidate_headings = [cand['normalized_heading'] for cand in  item['candidates'][0][1]]# rel heading
        ...
        for prob, cand in zip(cand_probs, candidate_headings):
            sam = (np.abs(diff_with_direction(cand, object_gt_heading)), prob, scan_id, step_id, path_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--store_dir", default='lmer_data', help="Path to the store directory.")
    ...
    obj_reverie_sample=load_jsonl(
        f'{input_path}/rxr_reverie_obj_3m_sample.jsonl')
    ...
    instr_id2no_end_reverie = {item['instruction_id']: item for item in no_end_reverie_sample}
    instr_id2intervene_result = {item['instr_id']: item for item in submit_obj_reverie_sample}
    instr_id2no_intervene_result = {item['instr_id']: item for item in submit_no_end_reverie_sample}


    filter_indexes = filter_instances_by_objects_heading(submit_obj_reverie_sample, instr_id2obj_reverie, fov_object=30)

    filtered_intervene = [item for item in submit_obj_reverie_sample if item['instr_id'] in filter_indexes]
    filtered_no_intervene = [item for item in submit_no_end_reverie_sample if item['instr_id'] in filter_indexes]

    intervene_format = create_scan_id2intervene_label2object(instr_id2obj_reverie, filtered_intervene, 1)
    no_intervene_format = create_scan_id2intervene_label2object(instr_id2no_end_reverie, filtered_no_intervene, 0)
